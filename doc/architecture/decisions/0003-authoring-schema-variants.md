...
Form variations are tackled through a mixture of routing rules (go here if...), skip conditions (skip this if...) and splitting of schemas into "form types" (e.g. Retail Sales Index (RSI) form types 0102, 0112 are survey runner schemas 1_0102.json and 1_0112.json).
Compared to existing surveys Monthly Business Survey (MBS) has a large number of form types which makes splitting and maintaining them by hand too onerous. The Author service is the long term solution but we need something more manageable in the interim for the MBS.
...
- A block file will define a single survey runner block and the question it contains. Blocks which have different metadata but contain the same question will duplicate the question.
Blocks which contain the same question should have the same file name with a suffix that makes identifying potential duplicate/similar questions easy, e.g. for RSI `total-internet-sales-0102`, `total-internet-sales-0112`. Blocks will be stored in YAML.
...
Questions do not have a single source of truth. A question can appear in many blocks, where typically the blocks have different routing and/or skip conditions. As questions are embedded in block files they are effectively duplicated.
Authors who are updating a question, e.g. a guidance change, will have to bear this in mind. They will have to identify the blocks that contain the question and update each of them.
