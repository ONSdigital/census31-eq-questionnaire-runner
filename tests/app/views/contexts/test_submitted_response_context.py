...
def test_build_view_submitted_response_context_summary(app: Flask):
    with app.app_context():
        questionnaire_store = fake_questionnaire_store({"tx_id": "tx_id", "ru_name": "Apple"}, SUBMITTED_AT)
        context = build_view_submitted_response_context("en", SCHEMA, questionnaire_store, SurveyType.DEFAULT)
        ...
        assert (
            context["summary"]["sections"][0]["groups"][0]["blocks"][0]["question"]["answers"][0]["value"]
            == "John Smith"
        )
        assert context["summary"]["sections"][0]["groups"][0]["blocks"][0]["question"]["title"] == "What is your name?"
        assert (
            context["summary"]["sections"][0]["groups"][1]["blocks"][0]["question"]["answers"][0]["value"] == "NP10 8XG"
        )
        assert (
            context["summary"]["sections"][0]["groups"][1]["blocks"][0]["question"]["title"] == "What is your address?"
        )
...
def test_view_submitted_response_context_submitted_text_without_ru_name(app: Flask, survey_type):
    with app.app_context():
        ...
        context = build_view_submitted_response_context("en", SCHEMA, questionnaire_store, survey_type)
...
def test_build_view_submitted_response_context_submitted_text_with_trad_as(app: Flask):
    with app.app_context():
        ...
        assert context["submitted_text"] == "Answers submitted for <span>Apple</span> (Apple Inc)"
