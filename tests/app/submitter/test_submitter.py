...
def test_rabbitmq_submitter_not_published_when_fails_to_connect_to_queue(rabbitmq_submitter, patch_blocking_connection):
    # Given
...
def test_rabbitmq_submitter_url_generation_with_credentials(patch_blocking_connection, patch_url_parameters, mocker):
    # Given
...
def test_rabbitmq_submitter_log_warning_message_when_fail_to_disconnect(rabbitmq_submitter, mocker):
    # Given
...
def test_rabbitmq_submitter_returns_false_when_fail_to_publish_message(rabbitmq_submitter, mocker):
    # Given
    channel = mocker.Mock()
    channel.basic_publish = mocker.Mock(side_effect=NackError("Mock exception for basic_publish"))
    connection = mocker.Mock()
...
def test_rabbitmq_submitter_metadata_is_sent_in_header_when_message_sent(rabbitmq_submitter, mocker):
    # Given
    channel = mocker.Mock()
    connection = mocker.Mock()
    connection.channel.side_effect = mocker.Mock(return_value=channel)
