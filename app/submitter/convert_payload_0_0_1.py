    ...
    *,
    ...
    for routing_path in full_routing_path:
        for block_id in routing_path:
            ...
            answers_in_block = data_stores.answer_store.get_answers_by_answer_id(answer_ids, routing_path.list_item_id)
            ...
            for answer_in_block in answers_in_block:
                ...
                for answer_id, answer in schema.get_answers_for_question_by_id(question).items():
                ...
                if answer_schema is not None and value is not None:
                    if answer_schema["type"] == "Checkbox":
                        data.update(
                            _get_checkbox_answer_data(data_stores.answer_store, answer_schema, value)  # type: ignore
                        )
...
def _format_downstream_answer(answer_type: str, answer_value: AnswerValueTypes, answer_data: str) -> str:
    if isinstance(answer_value, str):
        if answer_type == "Date":
            return datetime.strptime(answer_value, "%Y-%m-%d").replace(tzinfo=timezone.utc).strftime("%d/%m/%Y")

        if answer_type == "MonthYearDate":
            return datetime.strptime(answer_value, "%Y-%m").replace(tzinfo=timezone.utc).strftime("%m/%Y")

    return answer_data


def _get_checkbox_answer_data(answer_store: AnswerStore, answer_schema: Mapping, value: ListAnswer) -> dict[str, str]:
    qcodes_and_values = []
    for user_answer in value:
        # find the option in the schema which matches the users answer
        option = next(
            (option for option in answer_schema["options"] if option["value"] == user_answer),
            None,
        )
        ...
        if option:
            if "detail_answer" in option:
                ...
                if detail_answer:
                    # Ignore mypy type because the answer type can be non-string,
                    # but user_answer is expected to be a string.
