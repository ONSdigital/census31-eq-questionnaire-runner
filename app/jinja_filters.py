...
def get_min_max_value_width(min_max: Literal["minimum", "maximum"], answer: AnswerType, default_value: int) -> int:
    ...
    If the min or max for the answer is a value source but not an "answers"
    source, such as a calculated or grand calculated summary, use the length
    of the default value for the min and max width, as the actual min and max
    width cannot currently be determined
    """
...
def map_list_collector_config(
    ...
    for index, list_item in enumerate(list_items, 1):
        ...
        if related_answers:
            ...
            # Again, List Collector Content list summaries don't use the
            # row_item with "title" and are structured differently.
            if editable:
