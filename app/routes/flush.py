...
def _submit_data(user: User) -> bool:
    ...
    if questionnaire_store and questionnaire_store.data_stores.answer_store:
        ...
        # metadata is populated and can safely be used non-optionally.
        ...
        # Type ignore: Instance attribute 'eq' is a dict with key
        # "submitter" and value type GCSSubmitter, LogSubmitter, or
        # RabbitMQSubmitter.
...
def _get_converted_answers_message(
    ...
    Currently only v2 is supported, so
    `app.submitter.converter_v2.convert_answers_v2` is used.
    Returns:
        object: str
    """
    return json_dumps(
