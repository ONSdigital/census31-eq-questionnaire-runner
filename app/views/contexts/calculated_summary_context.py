...
class CalculatedSummaryContext(Context):
    ...
    def build_view_context_for_calculated_summary(self, current_location):
        ...
        formatted_total = self._get_formatted_total(groups or [], current_location=current_location)

        context = {
            "summary": {
                "groups": groups,
                "answers_are_editable": True,
                "calculated_question": self._get_calculated_question(block["calculation"], formatted_total),
    ...
    def _build_calculated_summary_section(self, rendered_block, current_location):
        ...
        blocks_to_calculate = [self._schema.get_block_for_answer_id(answer_id) for answer_id in answers_to_calculate]
        ...
        for block in unique_blocks:
            if QuestionnaireSchema.is_question_block_type(block["type"]):
                transformed_block = self._remove_unwanted_questions_answers(block, answers_to_calculate, current_location=current_location)
    ...
    def _remove_unwanted_questions_answers(self, block, answer_ids_to_keep, current_location):
        """
