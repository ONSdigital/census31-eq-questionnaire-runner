...
class PathFinder:
    ...
    def routing_path(self, section_key: SectionKey) -> RoutingPath:
        ...
        list_name = self.schema.get_repeating_list_for_section(current_location.section_id)

        if section:
            when_rules_block_dependencies = self.get_when_rules_block_dependencies(section_key.section_id)
    ...
    def get_when_rules_block_dependencies(self, section_id: str) -> list[str]:
        ...
        repeating sections.
        It is supported when the section is dependent i.e. the current section
        is repeating and building the routing path for sections that are not,
        It isn't supported if it needs to build the path for repeating sections"""
        dependencies_for_section = self.schema.get_all_when_rules_section_dependencies_for_section(section_id)

        return [
            block_id
            for dependent_section in dependencies_for_section
            for block_id in self.routing_path(SectionKey(dependent_section))
            if SectionKey(dependent_section) in self.data_stores.progress_store.started_section_keys()
        ]
    ...
    def _build_routing_path_block_ids(
        ...
        routing_path_block_ids: list[str] = []
        block_index = 0
        repeating_list = self.schema.get_repeating_list_for_section(current_location.section_id)
        ...
        while block_index < len(blocks):
            ...
            if not is_skipping:
                ...
                if repeating_list and current_location.list_item_id:
                    ...
                    this_location = Location(section_id=current_location.section_id, block_id=block_id)
    ...
    def _evaluate_routing_rules(
        ...
        # mutated; we don't want to update its memory reference.
        block_ids_for_dependencies = list(routing_path_block_ids) + when_rules_block_dependencies
        ...
        for rule in routing_rules:
            rule_valid = when_rule_evaluator.evaluate(when_rule) if (when_rule := rule.get("when")) else True
            ...
            if rule_valid:
                ...
                next_block_index = PathFinder._block_index_for_block_id(blocks, next_block_id)
                next_precedes_current = next_block_index is not None and next_block_index < block_index

                if next_precedes_current:
                    self._remove_current_blocks_answers_for_backwards_routing(rule, this_location)
    ...
    def _remove_current_blocks_answers_for_backwards_routing(self, rule: Mapping, this_location: Location) -> None:
        if block_id := this_location.block_id:
            answer_ids_for_current_block = self.schema.get_answer_ids_for_block(block_id)
            if "when" in rule:
    ...
    def _remove_block_answers_for_backward_routing_according_to_when_rule(
        ...
        for rule in operands:
            if isinstance(rule, dict) and ("identifier" in rule and rule["identifier"] in answer_ids_for_current_block):
