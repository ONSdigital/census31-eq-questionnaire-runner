...
class NumberHandler(FieldHandler):
    ...
    def get_field(self) -> Union[DecimalField, IntegerField]:
        field_type = DecimalFieldWithSeparator if self.max_decimals > 0 else IntegerFieldWithSeparator
        return field_type(label=self.label, validators=self.validators, description=self.guidance)
